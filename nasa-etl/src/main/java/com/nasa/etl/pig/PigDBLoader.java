package com.nasa.etl.pig.loader;
 ...
 * PigDBLoader
 *
 * Loads the TSV output files produced by the three Pig scripts into the
 * same relational tables used by the MapReduce pipeline (DBLoader).
 *
 * The output format of each Pig script is identical to MapReduce:
 *
 *   Q1 (q1_daily_traffic):
 ...
 *
 * All DDL is shared with DBLoader (tables are created with IF NOT EXISTS).
 */
public class PigDBLoader {
     ...
     *
     * Expected TSV format (produced by query1_daily_traffic.pig):
     *   batch_id \t log_date \t status_code \t request_count \t total_bytes
     */
    public static Map<Integer, Long> loadQuery1(Connection conn, String localQ1Dir, int runId)
            throws Exception {
        String sql =
        ...
        System.out.println("[PigDBLoader] Q1 rows loaded: " + loaded);
     ...
     *
     * Expected TSV format (produced by query2_top_resources.pig):
     *   batch_id \t resource_path \t request_count \t total_bytes \t distinct_host_count
     */
    public static void loadQuery2(Connection conn, String localQ2Dir, int runId)
        ...
        System.out.println("[PigDBLoader] Q2 rows loaded: " + loaded);
    }
     ...
     *
     * Expected TSV format (produced by query3_hourly_error.pig):
     *   batch_id \t log_date \t log_hour \t error_request_count \t
     *   total_request_count \t error_rate \t distinct_error_hosts
     */
    public static void loadQuery3(Connection conn, String localQ3Dir, int runId)
        ...
        System.out.println("[PigDBLoader] Q3 rows loaded: " + loaded);
    }
